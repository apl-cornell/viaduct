...
public class AnfConverter {
                ...
                Iterable<StatementNode>>
  {
    ...
    public Iterable<StatementNode> visit(LetBindingNode node) {
      final AnfStmtVisitor visitor = enter(node);

      // don't visit RHS of a let binding; because let bindings are not supported
      // in the parser, this means that all let bindings are generated by the compiler;
      // specifically, they would be generated by the ANF translator. this is a hacky
      // way to ensure that the ANF pass is idempotent.

      // final ExprResultT rhs = node.getRhs().accept(visitor.getExpressionVisitor());
      final ExpressionNode rhs = node.getRhs();

      final Iterable<StatementNode> result = leave(node, visitor, rhs);
    ...
    private final class AnfExprVisitor
      ...
      public ExpressionNode visit(ReadNode node) {
        final ReferenceNode reference =
            node.getReference().accept(atomicConverter.getReferenceVisitor());

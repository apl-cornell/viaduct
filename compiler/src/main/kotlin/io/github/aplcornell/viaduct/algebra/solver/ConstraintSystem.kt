...
class ConstraintSystem<C : HeytingAlgebra<C>, V, T : Throwable>(
     ...
     * and an edge from term `t2` to `t1` corresponds to the constraint `t1 ≤ f(t2)` where `f` is
     ...
     */
    private val bestEffortSolution: ConstraintSolution<C, V> =
        run {
    ...
    companion object {
        ...
        private fun <V, E> Graph<V, E>.addEdgeWithVertices(
            source: V,
            target: V,
            edge: E,
        ) {

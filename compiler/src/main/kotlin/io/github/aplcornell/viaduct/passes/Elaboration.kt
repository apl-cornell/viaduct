...
private class FunctionElaborator(val nameGenerator: FreshNameGenerator) {
    fun elaborate(functionDecl: SFunctionDeclarationNode): IFunctionDeclarationNode {
        ...
        val elaboratedParameters =
            functionDecl.parameters.map { parameter ->
                IParameterNode(
...
private class StatementElaborator(
    ...
    private fun copy(
        ...
        StatementElaborator(
            nameGenerator,
            // Temporaries are local and reset at each block.
            NameMap(),
            objectRenames,
            jumpLabelRenames,
            surroundingLoop,
    ...
    private fun freshTemporary(baseName: String? = null): Temporary = Temporary(nameGenerator.getFreshName(baseName ?: TMP_NAME))

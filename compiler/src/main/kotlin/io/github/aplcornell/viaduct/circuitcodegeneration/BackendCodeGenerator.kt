...
private class BackendCodeGenerator(
    ...
    fun generateClass(): TypeSpec {
        val classBuilder =
            TypeSpec.classBuilder(host.name.replaceFirstChar { it.uppercase() }).primaryConstructor(
                ...
                is LetNode -> {
                    when (val command = stmt.command) {
                        is CircuitCallNode -> {
                            ...
                            val inputs =
                                if (context.host in importingHosts) {
                                    val (importCode, inputs) =
                            ...
                            if (context.host in circuitHosts) {
                                val outNames =
                                    circuitDecl.outputs.associateWith { outParam ->
                                        val outName =
    ...
    fun generate(circuitDeclaration: CircuitDeclarationNode): FunSpec {
        ...
        val outParams =
            circuitDeclaration.outputs.map { param ->
